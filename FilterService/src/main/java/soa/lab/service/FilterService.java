...
public class FilterService {
    ...
    public List<OrganizationDto> filterOrgsByType(String type) {
        OrganizationDto[] responseBody = getOrganizationsFromMainService();
        log.info("Request from main service received with {} elements.\nFiltering by type.", responseBody.length);
        List<OrganizationDto> organizations = Arrays.stream(responseBody).filter(o -> o.getType().equals(type)).collect(Collectors.toList());
        if (organizations.isEmpty()) throw new DataNotFoundException("Organizations not found");
        return organizations;
    }

    public List<OrganizationDto> filterOrgsByEmployeesCount(int min, int max) {
        OrganizationDto[] responseBody = getOrganizationsFromMainService();
